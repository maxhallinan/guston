    ...
    describe "Parse" $ do
      describe "Parse.parseFile" $ do
        describe "special forms" $ do
          ...
          it "parses =" $ do "=" `parsesFileTo` [XExpr (SFrm Def) (Info (1, 1))]
          it "parses ==" $ do
        ...
        describe "symbol" $ do
          it "parses a symbol" $ do property prop_parseFile_Sym
        describe "list" $ do
          it "parses a list" $ do property prop_parseFile_List
      describe "Parser.parseStr" $ do
        describe "special forms" $ do
          ...
          it "parses ::" $ do "::" `parsesStrTo` XExpr (SFrm Cons) (Info (1, 1))
          it "parses if" $ do "if" `parsesStrTo` XExpr (SFrm If) (Info (1, 1))
          it "parses =" $ do "=" `parsesStrTo` XExpr (SFrm Def) (Info (1, 1))
          it "parses ==" $ do "==" `parsesStrTo` XExpr (SFrm IsEq) (Info (1, 1))
          it "parses fn" $ do
            "fn" `parsesStrTo` XExpr (SFrm Lambda) (Info (1, 1))
          it "parses quote" $ do
            "quote" `parsesStrTo` XExpr (SFrm Quote) (Info (1, 1))
        describe "symbol" $ do
          it "parses a symbol" $ do property prop_parseStr_Sym
        describe "list" $ do it "parses a list" $ do property prop_parseStr_List
...
token gen = do
  ...
  return (s1 ++ x ++ s2)
  where
    space = oneof [whitespace, lineComment]
...
isNotSFrm x = not $ elem x sFrms
  where
    sFrms = ["::", "=", "==", "atom?", "first", "fn", "if", "quote", "rest"]
...
symbol =
  ...
  where
    initial :: Gen Char
    initial = oneof [elements "!$%&*/:<=>?~_^", alpha]
    subsequent :: Gen Char
    subsequent = oneof [initial, numeric, elements ".+-"]
...
list = parens sexprs
  where
    sexprs = unwords <$> (resize 2 $ listOf sexpr')
